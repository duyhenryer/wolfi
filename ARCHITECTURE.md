...
Following [shyim/wolfi-php](https://github.com/shyim/wolfi-php) and official Wolfi patterns:

```
wolfi/
├── php-8.4/            # Package directories at root level
│   ├── php-8.4.yaml    # Melange package definition
...
│   ├── build.yaml      # Build & publish packages
│   ├── pr.yaml         # Lint & test on PR
│   ├── docker.yaml     # Build Docker images
│   ├── wolfictl-update-gh.yaml  # Auto-update from GitHub
│   └── wolfictl-update-rm.yaml  # Auto-update from Release Monitor
...
- ✅ Package directories at root (not nested in `packages/`)
- ✅ `/packages` reserved for build output only (gitignored)
- ✅ Each package is self-contained with YAML + docs
